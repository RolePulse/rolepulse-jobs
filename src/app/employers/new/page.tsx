...
function EmployerNewForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    router.push(`/employers/post?tier=${tier}&employer_id=${data.employer_id}`)
  }

  return (
    <div className="min-h-screen bg-rp-white flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <h1 className="text-3xl font-semibold text-rp-text-1 mb-2">Company details</h1>
        <p className="text-sm text-rp-text-3 mb-8">
          Tell us about your company so we can set up your posting.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="website" className="block text-sm font-medium text-rp-text-2 mb-1">
              Website <span className="text-rp-text-3 font-normal">(optional)</span>
            </label>
            <input
              id="website"
              type="url"
