...
function KanbanCard({
  ...
  return (
    <div
      ...
      onDragStart={selectMode ? undefined : onDragStart}
      onClick={selectMode ? () => onToggleSelect?.(app.id) : onClick}
      className={`bg-white rounded-xl border p-3 cursor-pointer hover:shadow-sm transition-all select-none ${selected ? 'border-rp-accent bg-orange-50/40 ring-1 ring-rp-accent/30' : 'border-rp-border hover:border-rp-accent/40'}`}
    >
      <div className="flex items-start gap-2">
        {selectMode && (
          <div className={`w-4 h-4 mt-1 rounded border-2 flex-shrink-0 flex items-center justify-center transition-colors ${selected ? 'bg-rp-accent border-rp-accent' : 'border-rp-border'}`}>
...
export default function PipelinePage() {
  ...
  function selectAllInStage(stage: Stage) {
    const stageIds = apps.filter(a => a.stage === stage).map(a => a.id)
    setSelectedIds(prev => {
      const next = new Set(prev)
  ...
  async function executeBulkAction(stage: Stage, stageDetail?: string) {
    ...
    try {
      ...
      if (res.ok) {
        const json = await res.json()
        const updatedMap = new Map<string, Application>(json.applications.map((a: Application) => [a.id, a]))
        setApps(prev => prev.map(a => updatedMap.get(a.id) ?? a))
    ...
    <div className="min-h-screen bg-rp-bg">
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${selectMode ? 'bg-orange-500 text-white' : 'bg-white/10 text-zinc-400 hover:text-white'}`}
              >
                {selectMode ? `${selectedIds.size} selected · Exit` : '☐ Select'}
              </button>
              {/* View toggle */}
      ...
      {selectMode && selectedIds.size > 0 && (
        <div className="bg-white border-b border-rp-border px-4 md:px-6 py-3 flex items-center gap-2 md:gap-3 flex-wrap">
          <span className="text-sm font-medium text-rp-text-1">{selectedIds.size} selected</span>
          <div className="h-4 w-px bg-rp-border" />
          <select
            className="border border-rp-border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-rp-accent"
            defaultValue=""
            onChange={e => {
              const val = e.target.value as Stage
              if (val) setBulkConfirm({ action: 'stage', stage: val })
              e.target.value = ''
            }}
          >
          ...
          <div className="bg-white rounded-t-2xl md:rounded-2xl w-full max-w-sm p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-2 rounded-lg border border-rp-border text-sm text-rp-text-2 hover:border-rp-text-3 transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-white text-sm font-medium transition-colors disabled:opacity-50 ${
                  bulkConfirm.action === 'archive' ? 'bg-red-500 hover:bg-red-600' : 'bg-rp-accent hover:bg-rp-accent-dk'
                }`}
              >
                {bulkSaving ? 'Processing…' : 'Confirm'}
              </button>
            </div>
          </div>
        </div>
